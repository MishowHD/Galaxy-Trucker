...
import it.polimi.ingsw.Model.Cards.Pirates.PiratesEffectState;
...
class Card_PiratesTest {
    ...
    void setUp() {
        ...
        try (InputStream input = getClass().getResourceAsStream("/it.polimi.ingsw/pirates.json")) {
            if (input == null) {
...
//PLayer 4
        ...
        mainGame.getGameState().getCardinuse().getState().getNextState();
        mainGame.getGameState().chooseToClaimReward(true, mainGame.getPlayer(giuseppe));
        mainGame.getGameState().chooseToClaimReward(true, mainGame.getPlayer(loreto));
        ...
        mainGame.getGameState().chooseHowToFaceMeteors(mainGame.getPlayer(giuseppe), new ArrayList<>(List.of(2, 2, 1, 2)), mainGame.getGameFlightBoard());
        assertNotEquals(SSTTypes.Tile_NonAccesiblePlace, mainGame.getPlayer(giuseppe).getMyShip().getShipMatrix()[1][5].getType());

        //dice is 5. big shot
        assertEquals(1, mainGame.getPlayer(giuseppe).getMyShip().getShipMatrix()[1][2].getNumCharges());
        assertNotEquals(SSTTypes.Tile_NonAccesiblePlace, mainGame.getPlayer(giuseppe).getMyShip().getShipMatrix()[1][1].getType());
        mainGame.getGameState().chooseHowToFaceMeteors(mainGame.getPlayer(giuseppe), null, mainGame.getGameFlightBoard());
        assertTrue(mainGame.getGameState().getCardinuse().getState() instanceof PiratesEffectState);
        mainGame.getGameState().getCardinuse().getState().getNextState();
        assertEquals(SSTTypes.Tile_NonAccesiblePlace, mainGame.getPlayer(giuseppe).getMyShip().getShipMatrix()[1][1].getType());

        //dice is 7
        assertNotEquals(SSTTypes.Tile_NonAccesiblePlace, mainGame.getPlayer(giuseppe).getMyShip().getShipMatrix()[1][3].getType());
        mainGame.getGameState().chooseHowToFaceMeteors(mainGame.getPlayer(loreto), new ArrayList<>(List.of(2, 2, 1, 2)), mainGame.getGameFlightBoard());
        mainGame.getGameState().chooseHowToFaceMeteors(mainGame.getPlayer(giuseppe), new ArrayList<>(List.of(2, 2, 1, 2)), mainGame.getGameFlightBoard());
        assertNotEquals(SSTTypes.Tile_NonAccesiblePlace, mainGame.getPlayer(giuseppe).getMyShip().getShipMatrix()[1][3].getType());
        assertEquals(0, mainGame.getPlayer(giuseppe).getMyShip().getShipMatrix()[1][2].getNumCharges());
    ...
    @Test
    void cardTestRefuse() throws Exception {
        EventBus b = mainGame.getEventBus();
        gameController = new GameController(mainGame, b);
        gameController.JoinGame("Loreto", 4);
        gameController.JoinGame("Giuseppe", 0);
...
//PLayer 4
        ...
        //PLAYER Sbaglito
        mainGame.getGameState().chooseToClaimReward(false, mainGame.getPlayer(giuseppe));
        mainGame.getGameState().chooseToClaimReward(false, mainGame.getPlayer(loreto));
    }
}
