...
import it.polimi.ingsw.Model.Cards.Planets.PlanetStateAddGoods;
import it.polimi.ingsw.Model.Cards.Planets.PlanetStateInit;
...
//PLayer 4
        ...
        assertTrue(mainGame.getGameState().getCardinuse().getState() instanceof PlanetStateInit);
        mainGame.getGameState().getCardinuse().getState().getNextState();
        //client sbagliato
        gameController.acceptToLandOnAPlanet(mainGame.getPlayer(giuseppe), true, 0);
        //deve printare errore
        //ora p giusto ma prima vedo che sono in stato giusto
        assertTrue(mainGame.getGameState().getCardinuse().getState() instanceof PlanetStateInit);
        //System.out.println("riga 549 il cli 1 può scendere!");
        gameController.acceptToLandOnAPlanet(mainGame.getPlayer(loreto), true, 0);

        //cambio stato
        assertTrue(mainGame.getGameState().getCardinuse().getState() instanceof PlanetStateAddGoods);
        mainGame.getGameState().getCardinuse().getState().getNextState();
//        TUIHandler tui = new TUIHandler();
//        tui.printCard(mainGame.getGameState().getCardinuse());
//        //setup dei goods con le pos
        ArrayList<Integer> xy = new ArrayList<>();
        ArrayList<ArrayList<Integer>> posizioni = new ArrayList<>();
        ArrayList<Goods> beni = new ArrayList<>();
        ArrayList<ArrayList<Goods>> pacchiDiBeni = new ArrayList<>();
        xy.add(2);
        xy.add(6);
        posizioni.add(xy);
        beni.add(new Goods(4, true));
        beni.add(new Goods(4, true));
        pacchiDiBeni.add(beni);
        ArrayList<ArrayList<Integer>> posizioni1 = new ArrayList<>();
        ArrayList<Integer> xy1 = new ArrayList<>();
        xy1.add(4);
        xy1.add(6);
        posizioni1.add(xy1);
        ...
        //quest0 deve andare bene
        assertTrue(mainGame.getGameState().getCardinuse().getState() instanceof PlanetStateAddGoods);
        gameController.chooseWhereToPutGoods(mainGame.getPlayer(giacomo), posizioni, pacchiDiBeni);
        gameController.chooseWhereToPutGoods(mainGame.getPlayer(loreto), posizioni1, pacchiDiBeni);
        gameController.chooseWhereToPutGoods(mainGame.getPlayer(loreto), posizioni, pacchiDiBeni);
        //controllo se ci sono goods nel posto buono
        //prendo i goods
        List<Goods> benieffective = mainGame.getPlayer(loreto).getMyShip().getShipMatrix()[2][6].getEffectivePresentGoods();
        ArrayList<SpaceShipTile> altretiles = new ArrayList<>();
        altretiles = mainGame.getPlayer(loreto).getMyShip().getShipBoardIterable().getTilesOfType(SSTTypes.Tile_CargoHold);
        int totGoodsInOtherTiles = 0;
        ArrayList<Goods> goodsinothertiles = new ArrayList<>();
        for (SpaceShipTile s : altretiles) {
            totGoodsInOtherTiles += s.getEffectivePresentGoods().size();
            goodsinothertiles.addAll(s.getEffectivePresentGoods());
        }
        mainGame.getGameState().getCardinuse().getPlanetVector();
        assertEquals(2, mainGame.getPlayer(loreto).getMyShip().getShipMatrix()[2][6].getEffectivePresentGoods().size()); //i beni che ho posizionato sono solo quelli
        for (Goods f : benieffective) { //tutti i beni che ho sono di val 4
            assertTrue(f.getValue() == 4);
            assertTrue(f.isRadioactive());//sono radioattivi
        }
        assertTrue(goodsinothertiles.isEmpty());
        assertEquals(0, totGoodsInOtherTiles);
        //ora che il cli ha preso e posizionato i suoi beni devo essere nello stato di attesa
        //cambio stato
        assertInstanceOf(PlanetStateInit.class, mainGame.getGameState().getCardinuse().getState());
        mainGame.getGameState().getCardinuse().getState().getNextState();
        //ora faccio parlare il secondo giocatore ma gli faccio scegliere il pianeta sbagliato
        gameController.acceptToLandOnAPlanet(mainGame.getPlayer(giuseppe), true, 0);
        //ora giusto
        gameController.acceptToLandOnAPlanet(mainGame.getPlayer(giuseppe), true, 1);
        //cambio stato
        assertInstanceOf(PlanetStateAddGoods.class, mainGame.getGameState().getCardinuse().getState());
        mainGame.getGameState().getCardinuse().getState().getNextState();
        //,ma faccio mettere i goods al terzo, mi aspetto errore
//        RuntimeException exception = assertThrows(RuntimeException.class, () ->
//                mainGame.getGameState().chooseWhereToPutGoods(mainGame.getPlayer(giacomo), posizioni, pacchiDiBeni)
//        );
//        assertEquals("Not correct player: expected 1, had 2", exception.getMessage());
        //ora parla giusto il secondo, gli dico di buttare tutto: in poss 0,0 goods a 0
        ArrayList<Integer> xy2 = new ArrayList<>();
        xy2.add(0);
        xy2.add(0);
        ArrayList<ArrayList<Integer>> posizione2 = new ArrayList<>();
        posizione2.add(null); //ho null, non voglio niente
        ArrayList<Goods> goods2 = new ArrayList<>();
        goods2.add(new Goods(0, false));

        ArrayList<ArrayList<Goods>> pacchiDiBeni2 = new ArrayList<>();
        pacchiDiBeni2.add(null);//ho null non voglio niente
        //gli dico che butto tutto
        gameController.chooseWhereToPutGoods(mainGame.getPlayer(giuseppe), null, null);
        //dovrebbe andare bene

        //cambio stato, prima vedo che siamo allo stato di init
        //assertTrue(mainGame.getGameState().getCardinuse().getState() instanceof PlanetStateInit);
        //gli ultimi due rifiutano,
        gameController.acceptToLandOnAPlanet(mainGame.getPlayer(giacomo), false, 0);
        gameController.acceptToLandOnAPlanet(mainGame.getPlayer(lorenzo), false, 0);

        //vedo stato
        //assertTrue(mainGame.getGameState().getCardinuse().getState() instanceof PlanetFinStat);
        //vedo che sono stati messi nella posizione giusta
        for (Player p : mainGame.getPlayers()) tui.printShipBoard(p.getMyShip());
        tui.printFlightBoard(mainGame.getGameFlightBoard());
        assertEquals(23, mainGame.getPlayer(giuseppe).getPositionOnBoard());
        assertEquals(4, mainGame.getPlayer(loreto).getPositionOnBoard());
