...
class Card_StardustTest {
    ...
    void setUp() {
        ...
        try (InputStream input = getClass().getResourceAsStream("/it.polimi.ingsw/stardust.json")) {
            if (input == null) {
...
//PLayer 4
        ...
        assertSame(SSTTypes.Tile_Cabin, mainGame.getPlayer(giacomo).getMyShip().getShipMatrix()[2][6].getType());
        assertSame(SSTTypes.Tile_NonAccesiblePlace, mainGame.getPlayer(giacomo).getMyShip().getShipMatrix()[1][6].getType());
        //considero di avere la cart
//        TUIHandler tui = new TUIHandler();
//        tui.printCard(mainGame.getGameState().getCardinuse());
        assertEquals(9, mainGame.getPlayer(lorenzo).getPositionOnBoard());
        assertEquals(14, mainGame.getPlayer(giacomo).getPositionOnBoard());
        assertEquals(13, mainGame.getPlayer(giuseppe).getPositionOnBoard());
