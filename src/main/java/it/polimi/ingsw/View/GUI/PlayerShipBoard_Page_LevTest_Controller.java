...
/**
 * The type Player ship board page lev test controller.
 */
public class PlayerShipBoard_Page_LevTest_Controller implements Initializable, GeneralView {
...
// Posticipa la prima chiamata a resizeBoard() e l’installazione del listener
            ...
            for (int col = 0; col < 7; col++) {
                ...
                cellContainer.setOnMouseClicked(e -> {
                    ...
                    if (selectingMeteorDefenses) {
                        // porto a 0-based interno: r 5→0,…9→4; c 4→0,…10→6
                    ...
                    if (selectingFirepowerCannons) {
                        ...
                        boolean fatto = false;
                        for (List<Integer> triplette : selectedFirepowerCannons) {
                            if (triplette.get(0) == r && triplette.get(1) == c) {
                                fatto = true;
                                triplette.set(2, triplette.get(2) + 1);  // OK perché triplette è un ArrayList
                                break;
                        ...
                        if (!fatto) {
                            ArrayList<Integer> newTriple = new ArrayList<>(List.of(r, c, 1));
                            selectedFirepowerCannons.add(newTriple);
                        }
                        highlightCell(cellContainer);
                        return;
                    }
    ...
    public void updateGoods(ShipBoard s,
        ...
        Platform.runLater(() -> {
            // 1) Rimuovo TUTTE le icone "good" da TUTTE le celle
            ...
            // 2) Per ogni storageTile, aggiungo le sue icone
            for (int idx = 0; idx < storageTiles.size(); idx++) {
                int row = storageTiles.get(idx).get(0);
                int col = storageTiles.get(idx).get(1);
                List<Goods> goodsList = newGoods.get(idx);
                StackPane cell = cellPanes[row][col];

                for (int i = 0; i < goodsList.size(); i++) {
                    Goods g = goodsList.get(i);
                    // ATTENZIONE: se i file si chiamano good1.png senza underscore usa questa stringa!
                    ...
                    // allineamento: distribuisco in basso
                    Pos align = switch (i) {
                        case 0 -> Pos.BOTTOM_LEFT;
                        case 1 -> Pos.BOTTOM_CENTER;
                        default -> Pos.BOTTOM_RIGHT;
                    };
                    ...
                    for (Node child : cell.getChildren()) {
                        ...
                        switch (type) {
                            ...
                            case "alien":
                                ...
                                iv.fitHeightProperty().bind(cell.heightProperty().multiply(0.9));
                                iv.fitWidthProperty().bind(iv.fitHeightProperty());
                                break;
        ...
        Platform.runLater(() -> {
            for (Node node : gridMatrix.getChildren()) {
                ...
                if (!cell.getChildren().isEmpty() && cell.getChildren().getFirst() instanceof ImageView iv) {
                    ...
                    if (url != null && url.contains("tileId_" + tile.getID() + ".jpg")) {
                        cell.getChildren().removeIf(n -> n instanceof ImageView overlay
                                && overlay.getImage().getUrl().contains("_alien.png"));
                        break;
                    }
                }
            }
            ...
            if (activatedCard != null) {
                ...
                try (InputStream imageStream = getClass().getResourceAsStream(imagePath)) {
                    if (imageStream != null) {
                        ...
                        imageView.setFitWidth(115 * 1.7);
                        imageView.setFitHeight(158 * 1.7);
                        imageView.setPreserveRatio(true);
    ...
    private void updatecardText() {
        String cardTextMessage = "";
        Card c = getLittleModelRepresentation().getActivatedcard();
        if (c instanceof Card_AbandonedShip) {
            cardTextMessage = "When you find an abandoned ship, a single player (starting with the leader and proceeding in flight-order) may choose to sacrifice the specified number of crew (humans and/or aliens) in exchange for the indicated number of cosmic credits. Electing to repair and claim the derelict also costs a certain number of flight days. Once one player takes the opportunity, all others are excluded.";
        } else if (c instanceof Card_AbandonedStation) {
            cardTextMessage = "Upon encountering an abandoned station, any player with at least the required number of crew (humans + aliens) may dock. The leader decides first; if they decline or lack sufficient crew, the chance passes in flight-order. The successful player immediately loads the indicated goods (redistributing or discarding cargo as needed) and then loses the specified number of flight days. No crew are lost when looting an abandoned station.";
        } else if (c instanceof Card_Combat_zone) {
            cardTextMessage = "A combat zone is resolved in three stages. First, the player with the fewest total crew (humans + aliens) loses three flight days. Second, the player with the lowest drive power (after optionally spending batteries for double engines, decided in flight-order starting with the leader) must remove two crew from their ship. Third, the player with the lowest firepower is threatened by one light and one heavy cannon shot coming from behind. For each shot, you roll two dice to determine the row or column of impact: a light shot can be blocked by an oriented shield (spending one battery), otherwise the component in that position is destroyed; a heavy shot cannot be blocked—only a very high or very low roll can miss entirely, otherwise the component is destroyed. In ties, the player farthest ahead on the flight track takes the penalty";

        } else if (c instanceof Card_Epidemic) {
            cardTextMessage = "An epidemic forces every player, in reverse flight-order (starting with the last), to remove one crew (human or alien) from each occupied cabin that is directly connected to another occupied cabin. To minimize losses, it is safest to avoid having adjacent occupied cabins.";

        } else if (c instanceof Card_MeteorSwarm) {
            cardTextMessage = "A meteor swarm consists of a sequence of small and large meteor strikes, resolved from top to bottom. For each small meteor, the leader rolls two dice to determine the target row or column; if the chosen tile on your ship is occupied, a well-built hull (with no exposed connector) will cause the small meteor to bounce off harmlessly. If it would hit an exposed connector, you may spend one battery to activate a shield; otherwise, the component hit is destroyed. For each large meteor, the same die roll indicates your target row or column, but shields cannot stop it. You can only destroy a large meteor if you have a forward-facing cannon in that column (a double cannon requires spending one battery to fire). Any component the meteor hits is destroyed if you do not shoot it.";

        } else if (c instanceof Card_OpenSpace) {
            cardTextMessage = "In open space, each player (starting with the leader and proceeding in flight-order) must declare their drive power. You may spend batteries to activate any double engines you have. Then you advance your rocket along the flight track by a number of empty spaces equal to your declared drive power, possibly overtaking other players (occupied spaces are skipped), and potentially becoming the new leader.";

        } else if (c instanceof Card_Pirates) {
            cardTextMessage = "When you encounter pirates, they attack in flight-order: first the leader, then the next player, and so on. Compare your ship’s firepower (spending batteries to fire double cannons if desired) to the pirate’s firepower shown on the card. If you defeat them, you choose either to take the indicated cosmic credits (losing the specified flight days) or to decline and let the pirates remain for the next player. If the pirates defeat you, your ship suffers cannon fire: roll two dice to determine the row or column of each shot (as indicated on the card), and apply the same resolution rules as for a light or heavy shot (a shield covers only light shots if oriented and activated with one battery; heavy shots cannot be blocked). All players defeated by that pirate card endure the same cannon shots, using the first defeated player’s die rolls.";

        } else if (c instanceof Card_Slavers) {
            cardTextMessage = "Slavers attack in flight-order, just like pirates, but their penalty upon victory is different: if the slavers defeat you, you must choose which crew (humans and/or aliens) to surrender to them in exchange for your freedom. If you defeat the slavers, you take the indicated cosmic credits (losing the specified number of flight days), and no other player can claim that reward.";

        } else if (c instanceof Card_Smugglers) {
            cardTextMessage = "Smugglers (Contrabbandieri) attack each ship in flight-order. The pirate icon in the top corner shows their firepower. Each player calculates their own firepower (spending batteries to fire double cannons if desired). If your firepower exceeds the smugglers’, you claim the goods shown on the card (loading the cubes onto your ship—redistribute or discard as needed) and then lose the specified flight days. In case of a tie, nothing happens and the smugglers move on. If your firepower is lower, you lose your two most valuable goods; if you have no goods, you instead lose one battery.";

        } else if (c instanceof Card_Stardust) {
            cardTextMessage = "Stardust is an event that makes every player lose one flight day for each exposed connector on their ship. Resolve in reverse flight-order (starting with the last): each player counts their total number of exposed connectors (each connector counts only once, regardless of single, double, or universal type) and moves their rocket backward along the track by that many empty spaces.";

        }

        cardText.setText(cardTextMessage);
